...
class NewMix extends React.Component {
  constructor(props) {
    super(props)
    this.state = {
      user: props.user,
  ...
  async componentDidMount() {
    const response = await axios.get(apiUrl + '/mixes/' + `${this.state.match.params.id}`,
      {
        headers:
        {
  ...
  handleChange = (event) => {
    const newMix = {...this.state.mix, [event.target.name]: event.target.value}
...
export default withRouter (NewMix)
